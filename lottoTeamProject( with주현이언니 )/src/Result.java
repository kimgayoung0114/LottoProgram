...
public class Result extends JFrame {
		...
		try (Connection conn = MySqlConnectionProvider.getConnection();
			...
			try (ResultSet rs = stmt.executeQuery()) {
				if (rs.next()) { // 현재 회차를 구매한 경우 현재 회차 당첨 번호 나옴
					...
					WinnerOrNot winnerChecker = new WinnerOrNot(round);
					winnerChecker.determineWinners();
					printWinningNumber();
					printPurchaseInfo();
	...
	private void extracted() {
		...
		lbl_5 = new JLabel(" ");
		lbl_5.setFont(font);
		lbl_5.setBounds(106, 506, 99, 38);
		getContentPane().add(lbl_5);

		lbl_4 = new JLabel(" ");
		lbl_4.setFont(font);
		lbl_4.setBounds(106, 462, 99, 38);
		getContentPane().add(lbl_4);

		lbl_3 = new JLabel(" ");
		lbl_3.setFont(font);
		lbl_3.setBounds(106, 418, 99, 38);
		getContentPane().add(lbl_3);

		lbl_2 = new JLabel(" ");
		lbl_2.setFont(font);
		lbl_2.setBounds(106, 373, 99, 38);
		getContentPane().add(lbl_2);

		lbl_1 = new JLabel(" ");
		lbl_1.setFont(font);
		lbl_1.setBounds(106, 329, 99, 38);
		...
		lblNewLabel_2.setBounds(211, 185, 113, 30);
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				Main main = new Main();
				main.setLocationRelativeTo(Result.this); // Main 클래스의 위치에 맞추어 창이 생성됨
	...
	private void generateRandomNumbers() {
	    try (Connection conn = MySqlConnectionProvider.getConnection()) {
	        // 이미 존재하는 회차인지 확인
	        String roundExistsQuery = "SELECT * FROM numberofwinners WHERE round = ?";
	        try (PreparedStatement pstmt = conn.prepareStatement(roundExistsQuery)) {
	            pstmt.setInt(1, round);
	            try (ResultSet rs = pstmt.executeQuery()) {
	                if (rs.next()) {
	                    System.out.println(round + "회차는 이미 존재합니다.");
	                    return;
	                }
	            }
	        }
	        ...
	        int bonus;
	        do {
	            bonus = (int) (Math.random() * 45) + 1; // 1부터 45까지의 난수 생성
	        } while (numbers.contains(bonus)); // 중복된 숫자가 나오지 않도록 확인

	        // numberofwinners 테이블에 회차 정보 추가
	        String insertQuery = "INSERT INTO numberofwinners (round, num1, num2, num3, num4, num5, num6, bonus) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
	        try (PreparedStatement stmt = conn.prepareStatement(insertQuery)) {
	            stmt.setInt(1, round);
	            for (int i = 0; i < sortedNumbers.size(); i++) {
	                stmt.setInt(i + 2, sortedNumbers.get(i));
	            }
	            stmt.setInt(8, bonus); // 보너스 값 설정
	            stmt.executeUpdate();
	        }

	        // 다음 회차 정보를 lottorounds 테이블에 추가
	        String insertRoundQuery = "INSERT INTO lottorounds VALUES (?)";
	        try (PreparedStatement stmt = conn.prepareStatement(insertRoundQuery)) {
	            stmt.setInt(1, round + 1);
	            stmt.executeUpdate();
	        }
